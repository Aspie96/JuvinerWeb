...
public class SectionE implements Serializable {
    ...
    public Section toSection(boolean withCategories, boolean withThreads, boolean withPosts, boolean withEmail, boolean withDescription) {
        List<Category> categories;
        if(withCategories) {
            categories = new ArrayList<>();
            this.categories.forEach(category -> categories.add(category.toCategory(withThreads, withPosts, false, withEmail, withDescription)));
        } else {
            categories = null;
        }
